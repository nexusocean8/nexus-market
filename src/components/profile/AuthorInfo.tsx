...
import { followAccount, getProfile } from '@/services/user';
...
export const AuthorInfo = ({ creator }: Props) => {
  ...
  const {
    ...
    queryFn: () => getProfile(creator, address),
  });

  const followMutation = useMutation({
    mutationFn: async (variables: { creator: string; follow: boolean }) => {
      const { creator, follow } = variables;

      if (!address) {
        throw new Error('Wallet not connected');
      }

      await followAccount({ address, creator, follow });
    ...
    onSuccess: ({ follow }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['creator', creator] });
    },
    onError: (err: any) => {
      console.log(err);
      if (err.status === 400) {
        toast.warn("Can't follow yourself.");
        return;
      }
      toast.error('Unable to follow account!');
    },
  });

  const handleShareProfile = async () => {
    await navigator.clipboard.writeText(
      `${window.location.origin}/profile/${creator}`
    );
  ...
  if (error || !user) return <Navigate to="/not-found" />;

  return (
    <>
      <Metadata title={`${user.username} | Nexus`} />
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              ...
              <div className="flex justify-center sm:justify-end gap-3">
                {user.address === address && (
                  ...
                  >
                    <Settings className="w-5 h-5 text-white group-hover:translate-x-0.5 transition-transform" />
                  </button>
                )}
                ...
                {user.address !== address && (
                  ...
                  >
                    {!user.isFollowing ? (
                      <UserPlus className="w-5 h-5 text-white group-hover:translate-x-0.5 transition-transform" />
                    ) : (
                      <UserMinus className="w-5 h-5 text-white group-hover:translate-x-0.5 transition-transform" />
                    )}
                  </button>
                )}
                ...
                <button
                  ...
                  className="w-11 h-11 sm:w-12 sm:h-12 bg-lavender-500 hover:bg-lavender-600 dark:bg-lavender-500 dark:hover:bg-lavender-600 rounded-xl transition-all duration-200 flex items-center justify-center group shadow-md"
                >
                  <Share2 className="w-5 h-5 text-white group-hover:translate-x-0.5 transition-transform" />
                </button>
              </div>
            </div>
