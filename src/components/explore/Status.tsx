...
const statuses = ['Buy Now', 'Auction'];

const formatSearchType = (str: string): string => {
  return str.toLowerCase().replace(/\s+/g, '-');
};

export const Status = ({ formState, setFormState }: StatusProps) => {
  const [hide, setHide] = useState(false);

  const handleClick = (status: string) => {
    const formatted = formatSearchType(status);
    setFormState((prev) => ({
      ...prev,
      status: formatted as ExploreNFTs['status'],
    }));
  };
        ...
        <div className="mt-6 flex flex-col space-y-4">
          {statuses.map((status) => (
            ...
            >
              <input
                type="checkbox"
                checked={formState.status === formatSearchType(status)}
